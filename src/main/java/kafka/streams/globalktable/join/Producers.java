...
public class Producers {
	...
	public static void main(String... args) {
		...
		//https://github.com/spring-cloud/spring-cloud-stream-samples/issues/68#issuecomment-488294797
		//Generate messages for topic region-top-exports, the topics from which stateStores will be materializedAS on app startup needs to be produced first to avoid stream timing issue.
		List<KeyValue<String, String>> regionTopExport = Arrays.asList(
				new KeyValue<>("asia", "rice"),
				new KeyValue<>("americas", "beef"),
				new KeyValue<>("asia", "chicken"),
				new KeyValue<>("europe", "milk"),
				new KeyValue<>("europe", "eggs"),
				new KeyValue<>("americas", "coffee"),
				new KeyValue<>("asia", "apples")
		);
		...
		KafkaTemplate<String, String> template2 = new KafkaTemplate<>(pf2, true);
		template2.setDefaultTopic("region-top-exports");

		for (KeyValue<String,String> keyValue : regionTopExport) {
			template2.sendDefault(keyValue.key, keyValue.value);
		}

		//Generate messages for topic user-clicks3
		...
		//Generate messages for topic user-regions
